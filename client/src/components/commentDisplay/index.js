...
class CommentDisplay extends Component {
    ...
    componentDidMount(){
        ...
        this.props.getAllLastActive() // call to getAllLastActive function in actions/index.js
    }

    getGravatar(email) {

        const cleanerEmail = this.cleanEmail(email);
        ...
        const hashFromImg = MD5(cleanerEmail);

        return 'http://www.gravatar.com/avatar/' + hashFromImg;
    }
    ...
    cleanEmail(email) {
        ...
        if (email !== undefined) {
            ...
            cleanerEmail = cleanerEmail.replace(/ /g, '');

            cleanerEmail = cleanerEmail.toLowerCase();
    ...
    render() {
        ...
        const popUpLastActive = (email) => {
            this.props.lastActiveTime.map((item,idx) => {

                if (email === item.email){
                    alert(email + ' was last active on ' +
                    item.date.toString() + ' ' +
                    item.time.toString());
                }
        ...
        if (this.props.comments[0] === undefined) { // if it is empty of comments

            postItems = <Container className="commentDisplayContainer"><Row className="item">Sorry, no comments to display...</Row></Container>
        }
        else { // if it is full of comments

            const mapItems = this.props.comments.map((item,idx) => {
                return (
                    <Row className="item" key={idx}>
                        <Col xs="2" sm="2" md="2" large="2" xl="2">
                            <Row>
                                <Col><Image className="gravatar" onClick={() => popUpLastActive(item.email)} src={this.getGravatar(item.email)} roundedCircle /></Col>
                            </Row>
                            <Row>
                                <Col className="underGravatar"></Col>
                            </Row>
                        </Col>
                        <Col xs="10" sm="10" md="10" large="10" xl="10">
                            <Row>
                                <Col xs="10" sm="10" md="10" large="10" xl="10" className="email">
                                    <a href={item.email} className="active">{this.cleanEmail(item.email)}</a>
                                </Col>
                                <Col xs="2" sm="2" md="2" large="2" xl="2" className="rating">
                                    <FaStar />
                                    <span className="ratingNumber">{item.rating}</span>
                                </Col>
                            </Row>
                            <Row>
                                <Col className="comment">{item.comment}</Col>
                            </Row>
                        </Col>
                    </Row>
                );
            ...
            postItems = <Container className="commentDisplayContainer">{mapItems}</Container>
        }

        return (
            <div className="App_content CommentDisplay_content">{postItems}</div>
        );
    }
}
